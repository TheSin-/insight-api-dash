<h1 align="center">Insight-api-terracoin</h1>

<div align="center">
  <strong>A Terracoin blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/terracoin/insight-api-terracoin">
    <img src="https://img.shields.io/travis/terracoin/insight-api-terracoin/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/insight-api-terracoin">
    <img src="https://img.shields.io/npm/v/insight-api-terracoin.svg?style=flat-square" alt="NPM version" />
  </a>
</div>


This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/terracoin/insight-ui-terracoin.
...
- [Resources](#resources)
- [License](https://github.com/terracoin/insight-api-terracoin/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g bitcore-node-terracoin@latest
bitcore-node-terracoin create mynode
cd mynode
bitcore-node-terracoin install insight-api-terracoin
bitcore-node-terracoin start
```

The API endpoints will be available by default at: `http://localhost:3001/insight-api-terracoin/`

### Prerequisites

- [Bitcore Node Terracoin 3.x](https://github.com/terracoin/bitcore-node-terracoin)

**Note:** You can use an existing Terracoin data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `terracoin.conf`, as well as a few other additional fields.
...
### Block
```
  /insight-api-terracoin/block/[:hash]
  /insight-api-terracoin/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-terracoin/block-index/[:height]
  /insight-api-terracoin/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-terracoin/rawblock/[:blockHash]
```
...
```
  /insight-api-terracoin/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-terracoin/tx/[:txid]
  /insight-api-terracoin/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-terracoin/rawtx/[:rawid]
  /insight-api-terracoin/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-api-terracoin/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-api-terracoin/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-api-terracoin/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-api-terracoin/addr/[:addr]/balance
  /insight-api-terracoin/addr/[:addr]/totalReceived
  /insight-api-terracoin/addr/[:addr]/totalSent
  /insight-api-terracoin/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-api-terracoin/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-api-terracoin/addrs/[:addrs]/utxo
  /insight-api-terracoin/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-api-terracoin/addrs/utxo
```
...
```
  /insight-api-terracoin/txs/?block=HASH
  /insight-api-terracoin/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-api-terracoin/txs/?address=ADDR
  /insight-api-terracoin/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-api-terracoin/addrs/[:addrs]/txs[?from=&to=]
  /insight-api-terracoin/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-api-terracoin/addrs/txs
```
...
POST method:
```
  /insight-api-terracoin/tx/sendix
```
POST params:
```
  rawtx: "signed transaction as hex string"
...
GET method:
```
  /insight-api-terracoin/gobject/list/proposal
```
...
GET method:
```
  /insight-api-terracoin/gobject/get/[:hash]
  /insight-api-terracoin/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
### Masternodes List
```
  /insight-api-terracoin/masternodes/list
```
### Validate Masternode
```
  /insight-api-terracoin/masternodes/validate/[:payee]
  /insight-api-terracoin/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-api-terracoin/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-terracoin/peer
```

### Status of the Bitcoin Network
```
  /insight-api-terracoin/status?q=xxx
```
...
### Utility Methods
```
  /insight-api-terracoin/utils/estimatefee[?nbBlocks=2]
```
...
- (Medium)[How to setup a Terracoin Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
