...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  return {
    ...
    height: info.height,
    version: (blockObj.header.nonce == 0 ? blockObj.header.version % 0x100 : blockObj.header.version),
...
BlockController.prototype.getPoolInfo = function(block) {
  var coinbaseBuffer = (typeof block.header !== 'undefined' && typeof block.header.auxPow !== 'undefined' ? block.header.auxPow : block.transactions[0]).inputs[0]._scriptBuffer.toString('utf-8');

  for(var k in this.poolStrings) {
    if (coinbaseBuffer.match(k)) {
      return this.poolStrings[k];
    }
  }

  var match = coinbaseBuffer.match(/\/([ A-Za-z0-9._-]{1,25})\//);
  if (match !== null) {
    return {poolName: match[1]};
  }
...
BlockController.prototype.getBlockReward = function(height) {
  var halvings = Math.floor(height / 1050000);
